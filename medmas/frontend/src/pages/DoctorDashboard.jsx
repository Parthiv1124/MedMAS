              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button type="button" onClick={async () => {
                    ...
                    setRxLoading(true);
                    try {
                      const res = await fetch(`${API_BASE}/api/prescriptions/suggest`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ case_id: activeCase.id }),
                      });
                      ...
                      if (data.suggestion) {
                        const meds = data.suggestion.medications?.length > 0 ? 
                          data.suggestion.medications : [{ name: "", dosage: "", frequency: "", duration: "" }];
                        setRxForm(p => ({
                          ...p,
                          diagnosis: data.suggestion.diagnosis || p.diagnosis,
                          instructions: data.suggestion.instructions || p.instructions,
                          follow_up_date: data.suggestion.follow_up_date ? 
                            new Date(Date.now() + (parseInt(data.suggestion.follow_up_date) * 24 * 60 * 60 * 1000)).toISOString().split('T')[0] : 
                            p.follow_up_date,
                          medications: meds,
                        }));
                        console.log("RxForm updated with:", data.suggestion);
                      }
                    } catch (e) {
                  ...
                  }} disabled={rxLoading || !activeCase}
                    className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-lg font-medium hover:bg-indigo-200 disabled:opacity-50">
                    {rxLoading ? "AI Analyzing..." : "🤖 AI Suggest"}
                  </button>
                </div>
